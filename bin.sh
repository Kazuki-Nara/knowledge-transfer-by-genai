...
echo ""
echo "==========================================================================="
echo "  Knowledge Transfer by Generative AI"
echo "---------------------------------------------------------------------------"
echo "  This script deploys the Knowledge Transfer application using AWS CodeBuild."
echo "  No local environment dependencies - deployment runs entirely in AWS."
echo ""
echo "  ⚠️  WARNING: This will deploy resources to your AWS account."
echo "     For production environments, consider using IP restrictions."
echo "==========================================================================="
echo ""

# Prepare working directory (ensure idempotency)
WORK_DIR="knowledge-transfer-deploy-$(date +%s)"
if [ -d "$WORK_DIR" ]; then
...
git clone https://github.com/aws-samples/knowledge-transfer-by-genai.git "$WORK_DIR"
if [ $? -ne 0 ]; then
...
REPO_URL="https://github.com/aws-samples/knowledge-transfer-by-genai.git"
BRANCH="main"
GIT_TAG=""
...
while [[ "$#" -gt 0 ]]; do
  case $1 in
    ...
    --tag) GIT_TAG="$2"; shift ;;
    --region) DEPLOYMENT_REGION="$2"; shift ;;
...
StackName="KnowledgeTransferCodeBuildDeploy"
...
distributionUrl=$(echo "$logs" | grep -o 'KnowledgeTransferStack.DistributionUrl = [^ ]*' | cut -d' ' -f3 | tr -d '\n,')
...
echo "---------------------------------------------------------------------------"
echo "  Frontend URL: $distributionUrl"
echo ""
